  ...
  return (
    <div className="relative" ref={filterRef}>
      <button
        ...
        className={`inline-flex items-center px-4 py-2 border rounded-md border-[2px] ${
          isDark
            ? 'bg-DarkBG3 border-DarkBG3 text-TextLight hover:bg-DarkBG1'
            : 'bg-LightBG1 border-LightBG2 text-TextDark hover:bg-LightBG2'
        }`}
      >
        <FunnelIcon className={`h-5 w-5 text-IconColor ${language === "en" ? 'mr-2 ' : 'ml-2 ' }`} />
        {t['filters']} 
      </button>

      {isOpen && (
        <div className={`absolute ${language === "en" ? 'right-0' : 'left-0' } mt-2 w-64 rounded-md shadow-lg ${
          isDark ? 'bg-DarkBG3' : 'bg-LightBG3'

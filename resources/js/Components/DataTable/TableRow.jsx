...
const renderActions = () => {
    ...
    return (
        ...
        >
            ...
            {columns.map((column) => (
                <td key={column.key} className="px-6 py-4 whitespace-nowrap text-sm sm:px-2 sm:py-1">
            ...
            {buttons && buttons.map((button, index) => (
                button.show && button.show(row) && (
                    <button
                        ...
                        className={`m-2 inline-flex items-center px-2 py-2 ${button.bgColor} border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:${button.hoverColor} focus:outline-none focus:ring-2 focus:${button.ringColor} focus:ring-offset-2 transition ease-in-out duration-150`}
                    >
                        {button.icon}
                        {button.label}
                    </button>
